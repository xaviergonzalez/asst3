...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    cudaMalloc(&device_x, sizeof(float) * N);  // the value of device_x is the address of memory. I need to change this value, so I have to pass the address of device_x
    ...
    printf("CUDA saxpy kernel time: %.3f ms\n", 1000.f * kernelDuration);
